...
#
# libssh2's vendored `openssl.h` still does a bare `#include <openssl/...>`,
# and a framework slice only contributes `-F`. `Sources/libssh2/openssl-shim/`
# bridges that: one forwarding header per OpenSSL header, reachable through a
# private `.headerSearchPath` in Package.swift. It is not a public headers
# path, so it is never copied anywhere a consumer could see it, and the
# canonical `openssl/...` tree stays free for another SDK's crypto.
#
...
# ---------------------------------------------------------------------------
# Stage 7: regenerate the private libssh2 header shim.
#
# One forwarding header per OpenSSL header, so libssh2's bare
# `#include <openssl/X.h>` reaches the framework. Reached through a private
# `.headerSearchPath` in Package.swift, never through a public headers path.
# ---------------------------------------------------------------------------

SHIM_DIR="$ROOT_DIR/Sources/libssh2/openssl-shim/openssl"
rm -rf "$ROOT_DIR/Sources/libssh2/openssl-shim"
mkdir -p "$SHIM_DIR"

shim_count=0
for header in "$BUILD_DIR/slice-macos/include/openssl"/*.h; do
...
// whose headers live under $HEADER_DIR_NAME/ so they cannot collide with another
// SDK's openssl/ tree. See the Stage 4 note in that script.
#pragma once
#include <OpenSSLCrypto/$HEADER_DIR_NAME/$base>
SHIM
  shim_count=$((shim_count + 1))
done

echo "Built dynamic OpenSSL XCFrameworks in $OUTPUT_DIR ($shim_count shim headers regenerated)"
